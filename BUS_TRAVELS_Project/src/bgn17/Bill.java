...
public class Bill {
	public static void printBill(Bus bus,String boardingStation, String destination,List<Passenger> passengersList,LocalDate date) {
		printHorizontal();
		System.out.printf("%n|%38s%36s%n", "Bill", "|");
		printHorizontal();
		System.out.printf("%n| %-10s%-20S-%20S%7S%13S%2s", bus.getBusNumber(), boardingStation,destination,bus.getCategory(), bus.getBusType(), "|");
	...
	private static void printPassengers(List<Passenger> passengersList) {
		...
		for(int i=0;i<passengersList.size();i++) {
			...
			System.out.printf("%n|%74s%n| %-20s%-7d%-7S%-15S%24s", "|", passengersList.get(i).getName(),passengersList.get(i).getAge(), passengersList.get(i).getGender(), ageCategory, "|"); 		
		} 
	} 
