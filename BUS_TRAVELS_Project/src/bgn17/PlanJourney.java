...
public class PlanJourney {
	...
	public boolean bookTicket() {
		...
		while(!isValidStation(boardingStation)) {
			System.out.println("\nOh Oh!! We Couldn't Find Entered Station(-_-)");
		...
		System.out.println("\nEnter Date Of Your Journey in 'YYYY-MM-DD Format': ");
		...
		while(!isValidPassengerCount(passengerBus, passengersCount)) {
			System.out.println("\nRe-Enter Valid Number of Passengers: ");
		...
		while(true) {
			...
			case 1:
				if(isHuman()) {
					...
					System.out.println("\nThank You !!! Your Booking was Confirmed !!");
					return true;
				}
				break;
			case 2:
				System.out.println("\nSorry We Couldn't Confirm Your Booling !!");
				return false;
			default:
	...
	private static boolean isValidDate(String Date) {
		try {
			...
			if(inputDate.isBefore(LocalDate.now())) {
				System.out.println("Enter Date is Behind Today !!\n");
				return false;
			}
	...
	private static List<Bus> getPassengerBus(List<Bus> availableBusList, String busNum){
		return availableBusList.stream().filter(bus -> bus.getBusNumber().equals(busNum)).collect(Collectors.toList());
	}
	
	private static boolean isValidPassengerCount(Bus passengerBus, int passengersCount) {
		if(passengersCount > 6) {
			System.out.println("\nYou Can't Choose a Max of 6 passenger in a Single Transaction");
			return false;
		}else if(passengersCount < 1) {
			System.out.println("\nYou Should Choose Atleast 1 passenger to Book Tickets");
			return false;
		}else if(passengerBus.getVacancies() < passengersCount){
			System.out.println("\nSorry We Have Only" + passengerBus.getVacancies()+" Seates Left :)");
			return false;
		}
		return true;
	} 
	...
	private static List<Passenger> storePassenger(int passengerCount){
		...
		for(int i=1;i<= passengerCount;i++) {
			System.out.println("\nPlease Enter "+i+" Passenger Name: ");
			String name=sc.nextLine();
			System.out.println("\nPlease Enter "+i+" Passenger Gender: ");
			String gender=sc.nextLine();
			System.out.println("\nPlease Enter "+i+" Passenger Age: ");
			int age=sc.nextInt();
			sc.nextLine();
	...
	private static String generateCaptcha() {
		...
		for(int i=1;i<=6;i++) {
			captcha.append(chars.charAt(rand.nextInt(67)));
		}
